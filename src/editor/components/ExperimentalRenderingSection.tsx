  ...
  return (
    ...
    >
      <p className="section-intro">
        These GPU effects are ready to try, save, record, and export. They need
        Phaser WebGL; Canvas-only devices safely show the ordinary image.
      </p>
      <div
        ...
        {effects.visualMask.enabled && (
          <>
            ...
            <p className="inspector-hint inspector-hint--warning">
              ...
              follows this copy for its existing lifetime; it does not add
              another Timeline. Bake essential clipping into the source image
              when Canvas must match.
            </p>
          </>
        )}
      </div>
        ...
        {effects.animatedShine.enabled && (
          <FieldGroup>
            ...
            <RangeField
              ...
              onChange={(gradient) => change("animatedShine", { gradient })}
            />
          </FieldGroup>
        )}
          ...
          <>
            ...
            <FieldGroup>
              <RangeField
                ...
                unit="% of life"
                help="The point in every copy's existing lifetime where erasing begins."
                onChange={(value) =>
                  change("directionalDissolve", { start: value / 100 })
                }
              />
              <RangeField
                label="Erase finishes"
            ...
            <Toggle
              ...
              onChange={(reverse) => change("directionalDissolve", { reverse })}
            />
            {effects.directionalDissolve.pattern === "noise" && (
              <p className="inspector-hint inspector-hint--warning">
                WebGL only: Canvas keeps the ordinary, un-eroded sprite. Noise
                erosion adds one GPU pass per visible copy. Shine, blur, and
                glow follow its remaining silhouette. Add a normal opacity fade
                when the Canvas fallback must still disappear.
              </p>
            )}
          </>
        )}
      ...
      <div className="experimental-effects-group">
        ...
        {effects.spriteWarp.enabled && (
          <>
            ...
            <p className="inspector-hint inspector-hint--warning">
              This is sprite-local distortion. True heat haze or refraction that
              bends the scene needs explicit game-camera capture and is
